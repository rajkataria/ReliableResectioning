...
MDS::MDS(const std::string database_path, std::string metric, const DatabaseCache* database_cache) {
	std::cout << "***********************************************************************************************************************************" << std::endl;
    std::cout << "******************************************************** Initializing MDS *********************************************************" << std::endl;
    ...
    SceneGraph sg = database_cache->SceneGraph();
    for (auto const& im1 : image_names) {
        ...
        for (auto const& im2 : image_names) {
            if (im1.compare(im2) == 0) {
                ...
                im1_aam_map.insert(std::make_pair(im2, aam_distances_map.find(im2)->second.find(im1)->second));
            } else {
                Image im1_ = database.ReadImageWithName(im1);
                Image im2_ = database.ReadImageWithName(im2);
                if (!sg.ExistsImage(im1_.ImageId()) || !sg.ExistsImage(im2_.ImageId())) {
                    continue;
                }
                std::vector<std::pair<point2D_t, point2D_t>> correspondences = sg.FindCorrespondencesBetweenImages(im1_.ImageId(), im2_.ImageId());

                std::vector<std::pair<point2D_t, point2D_t>> :: iterator correspondence_itr;
                double aam_matches = 0.0;
                for (correspondence_itr = correspondences.begin(); correspondence_itr != correspondences.end(); ++correspondence_itr) {
                    std::vector<SceneGraph::Correspondence> point_correspondences = sg.FindCorrespondences(im1_.ImageId(), (*correspondence_itr).first);
                    if (!sg.HasCorrespondences(im1_.ImageId(), (*correspondence_itr).first)) {
                        continue;
                    }
                    if (!sg.HasCorrespondences(im2_.ImageId(), (*correspondence_itr).second)) {
                        continue;
                    }
    ...
    std::cout << "******************************************************** Finished Initializing MDS ************************************************" << std::endl;
}
...
float MDS::GetMinImageDistanceScore(const image_t image_id, const Image& image) const {
    float min_aam_distance = 100000.0;
    std::string im1 = image.Name().c_str();
    std::string min_aam_distance_image;
    std::map<std::string, float> im1_aam_map = aam_distances_map.find(im1)->second;
  
    for (int j=0; j < image_names.size(); j++) {
        std::string im2 = image_names[j];
        float im1_im2_aam_distance = im1_aam_map.find(im2)->second;
        bool find_bool = std::find(registered_images_.begin(), registered_images_.end(), im2) != registered_images_.end();

        if ((im1_im2_aam_distance < min_aam_distance) && (im1.compare(im2) != 0) && (std::find(registered_images_.begin(), registered_images_.end(), im2) != registered_images_.end())) {
...
int MDS::RegisteredImageRank(const Image& im1, const Image& im2) const {
    ...
    std::map<std::string, float> im1_aam_map = aam_distances_map[im1.Name().c_str()];
    float im2_aam_distance = im1_aam_map[im2.Name().c_str()];
    
    std::set<std::string> :: iterator itr;
    for (itr = registered_images_.begin(); itr != registered_images_.end(); ++itr) {
...
int MDS::AdaptiveK(const Image& im1) const {
    ...
    for (itr = registered_images_.begin(); itr != registered_images_.end(); ++itr) {
        if (im1_aam_map[*itr] <= tau_value * closest_registered_image_aam_distance) {
            k_aam += 1;
        }
    }
    return k_aam;
